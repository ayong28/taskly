...
14. ✅ Add Description and Due Date fields to card creation and editing — Description is a plain `<textarea>` with no hard character cap (Trello itself has none); Due Date is a native `<input type="date">`. Settable in both `NewCardModal` (creation) and `CardModal` (edit, replacing the old title-only `renameCard` with `updateCard`).
15. ✅ Re-designed the app's UI applying the Step 12 design system across every component: Sidebar shell, board header/filter bar/list chrome, card tiles/labels, and both card modals. Done in four visually-inspected stages; also fixed a pre-existing Step 13 E2E regression (six specs still targeted the old inline card-title input) surfaced while verifying the full Playwright suite post-redesign.
